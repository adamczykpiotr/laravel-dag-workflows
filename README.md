...
Notes:
- This example mirrors the original `.tinker` workflow structure: a single fetch task, a group of parsing tasks (the group depends on the fetch), and chained sync tasks that depend on parsing tasks and earlier syncs.
- The example uses concrete job classes (e.g. `App\\Jobs\\DownloadFeedsJob`, `App\\Jobs\\ParseCatalogsJob`, `App\\Jobs\\SyncCatalogAlbumRelationsJob`) to better reflect a real application — replace these with your own job implementations as needed.
- Run this inside `php artisan tinker` or store it as a `.tinker` helper.
